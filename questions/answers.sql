...
with most_ordered_items as
				( select id as product_id, name as product_name, sum(quantity) as num_times_in_successful_orders
	...
	most_ordered_items_rank as
				( select *,
						rank() over(order by num_times_in_successful_orders desc ) row_rank
...
with order_quantity as
	...
	spender_rank as
				( select customer_id,
						location,
						total_spend,
						rank() over(order by total_spend desc ) ROW_RANK
					from spender
...
with location_count as
	...
	location_count_rank as
				( select location,
						checkout_count,
						rank() over(order by checkout_count desc ) row_rank
					from location_count)
select location,
	checkout_count
